...
/** Functions relating to temporal query */
public class TemporalQueryFuncs {
    ...
    public static String subjectToString(Node s) {
        ...
        if ( ! (s.isURI() || s.isBlank() ) )
            throw new TemporalIndexException("Found a subject that is not a URI nor a blank node: "+s) ;
    ...
    public static String graphNodeToString(Node g) {
        ...
        if ( ! (g.isURI() || g.isBlank() ) )
            throw new TemporalIndexException("Found a graph label that is not a URI nor a blank node: "+g) ;
    ...
    public static Entity entityFromQuad(EntityDefinition defn , Node g , Node s , Node p , Node o ) {
        ...
        if ( field == null )
            return null ;
        if ( !o.isLiteral() ) {
            Log.warn(TemporalQuery.class, "Not a literal value for mapped field-predicate: " + field + " :: "
                     + FmtUtils.stringForString(field)) ;
            return null ;
        }
        String x = TemporalQueryFuncs.subjectToString(s) ;
        String graphText = TemporalQueryFuncs.graphNodeToString(g) ;

        RDFDatatype datatype = o.getLiteral().getDatatype() ;
        Entity entity = new Entity(x, graphText, datatype) ;
