...
import static org.apache.jena.query.text.assembler.TemporalVocab.NS ;
...
{
    /*
<#entMap> a temporal:EntityMap ;
    temporal:entityField      "uri" ;
    temporal:defaultField     "temporal" ;
    temporal:map (
         [ temporal:field "temporal" ; temporal:predicate rdfs:label ]
         [ temporal:field "type" ; temporal:predicate rdfs:type  ]
         ) .
     */
    ...
    public EntityDefinition open(Assembler a, Resource root, Mode mode)
    {
        ...
        if ( results.size() == 0 ) {
            ...
            throw new TemporalIndexException("Failed to find a valid EntityMap for : "+root) ;
        }

        if ( results.size() !=1 )  {
            Log.warn(this, "Multiple matches for EntityMap for : "+root) ;
            throw new TemporalIndexException("Multiple matches for EntityMap for : "+root) ;
        }
        ...
        Statement listStmt = root.getProperty(TemporalVocab.pMap);
        while (listStmt != null) {
            RDFNode n = listStmt.getObject();
            if (! n.isResource()) {
                throw new TemporalIndexException("Text list node is not a resource : " + n);
            }
            ...
            if (listEntryStmt == null) {
                throw new TemporalIndexException("Text map list is not well formed.  No rdf:first property");
            }
            n = listEntryStmt.getObject();
            if (! n.isResource()) {
                throw new TemporalIndexException("Text map list entry is not a resource : " + n);
            }
            Resource listEntry = n.asResource();

            Statement fieldStatement = listEntry.getProperty(TemporalVocab.pField);
            if (fieldStatement == null) {
                throw new TemporalIndexException("Text map entry has no field property");
            }
            n = fieldStatement.getObject();
            if (! n.isLiteral()) {
                throw new TemporalIndexException("Text map entry field property has no literal value : " + n);
            }
            String field = n.asLiteral().getLexicalForm();

            Statement predicateStatement = listEntry.getProperty(TemporalVocab.pPredicate);
            if (predicateStatement == null) {
                throw new TemporalIndexException("Text map entry has no predicate property");
            }
            n = predicateStatement.getObject();
            if (! n.isURIResource()) {
                throw new TemporalIndexException("Text map entry predicate property has non resource value : " + n);
            }
            mapDefs.put(field, n.asNode()) ;

            Statement analyzerStatement = listEntry.getProperty(TemporalVocab.pAnalyzer);
            if (analyzerStatement != null) {
                n = analyzerStatement.getObject();
                if (! n.isResource()) {
                    throw new TemporalIndexException("Text map entry analyzer property is not a resource : " + n);
                }
        ...
        if ( defaultField != null ) {
            ...
            if ( c.isEmpty() )
                throw new TemporalIndexException("No definition of primary field '"+defaultField+"'") ;
        }
