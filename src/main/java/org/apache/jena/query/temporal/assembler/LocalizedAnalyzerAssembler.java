...
public class LocalizedAnalyzerAssembler extends AssemblerBase {
    ...
    public Analyzer open(Assembler a, Resource root, Mode mode) {
        if (root.hasProperty(TemporalVocab.pLanguage)) {
            RDFNode node = root.getProperty(TemporalVocab.pLanguage).getObject();
            if (! node.isLiteral()) {
                throw new TemporalIndexException("temporal:language property must be a string : " + node);
            }
