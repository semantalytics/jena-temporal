    ...
    */
    ...
    public static boolean open(Assembler a, Resource list) {
        ...
        while (current != null && ! current.equals(RDF.nil)){
            ...
            if (adding.hasProperty(TemporalVocab.pFilter)) {
                Statement filterStmt = adding.getProperty(TemporalVocab.pFilter);
                RDFNode filterNode = filterStmt.getObject();
                if (!filterNode.isResource()) {
                    throw new TemporalIndexException("addFilters temporal:filter must be a filter spec resource: " + filterNode);
                }
                
                // calls GenericFilterAssembler
                FilterSpec filterSpec = (FilterSpec) a.open((Resource) filterNode);
                
                if (adding.hasProperty(TemporalVocab.pDefFilter)) {
                    Statement defStmt = adding.getProperty(TemporalVocab.pDefFilter);
                    Resource id = defStmt.getResource();
                    
                    if (id.getURI() != null) {
                        ConfigurableAnalyzer.defineFilter(id.getURI(), filterSpec);
                    } else {
                        throw new TemporalIndexException("temporal:defineFilters temporal:defineAnalyzer property must be a non-blank resource: " + adding);
                    }
                } else {
                    Log.warn("DefineFiltersAssembler", "Filter specified but no temporal:defineFilter so filter is not accessible!");
                }
            }
