...
import org.apache.jena.query.temporal.TemporalIndexConfig;
...
/** Build a temporal search dataset */
public class JenaTextExample1
{
    ...
    public static Dataset createCode() 
    {
        ...
        // Build a temporal dataset by code.
        ...
        Dataset ds = TemporalDatasetFactory.createLucene(ds1, dir, new TemporalIndexConfig(entDef)) ;
    ...
    {
        log.info("Construct temporal dataset using an assembler description") ;
        // There are two datasets in the configuration:
        // the one for the base data and one with temporal index.
        // Therefore we need to name the dataset we are interested in. 
        Dataset ds = DatasetFactory.assemble("temporal-config.ttl", "http://localhost/jena_example/#text_dataset") ;
        return ds ;
    }
    ...
    public static void queryData(Dataset dataset)
    {
        ...
        String qs = StrUtils.strjoinNL
            ( "SELECT * "
            , " { ?s temporal:query (rdfs:label 'X1') ;"
            , "      rdfs:label ?label"
            , " }") ; 
