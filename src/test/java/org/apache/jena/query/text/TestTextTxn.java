 ...
 * {@link DirectoryReader#open(IndexWriter)}. jena-temporal does not currently do this.
 * <p>
 * When used outside a transaction, writes are "autocommit" (see
 * {@link TemporalDocProducerTriples}) and so are visible immediately.
 * <p>
...
public class TestTextTxn
{
    ...
    private Dataset create() {
        ...
        TemporalIndex tidx = new TemporalIndexChronix(dir, new TemporalIndexConfig(eDef)) ;
        Dataset ds = TemporalDatasetFactory.create(ds1, tidx) ;
        return ds ;
    }
    ...
    @Test public void textTxn_query_1() {
        ...
        Txn.executeRead(ds, ()->{
            String qs = StrUtils.strjoinNL("PREFIX temporal: <http://jena.apache.org/temporal#>",
                "PREFIX rdfs:    <http://www.w3.org/2000/01/rdf-schema#>",
                "SELECT *",
                "{ ?s temporal:query 'foo' ;",
                "     rdfs:label 'foo'",
                "}"
                ) ;
    ...
    @Test public void textTxn_from_namedGraph_query_1() {
        ...
        Txn.executeRead(ds,  ()->{
            String qs = StrUtils.strjoinNL(
                ...
                "SELECT *",
                "FROM <ex:g1>",
                "{ ?s temporal:query 'foo' . ?s rdfs:label ?o }"
                ) ;
            Query q = QueryFactory.create(qs) ;
            QueryExecution qexec = QueryExecutionFactory.create(q, ds) ;
            ResultSet rs = qexec.execSelect() ;
    ...
    @Test public void textTxn_subject_bound_first() {
        ...
        Txn.executeRead(ds,  ()->{
            String qs = StrUtils.strjoinNL(
                ...
                "PREFIX rdfs:   <http://www.w3.org/2000/01/rdf-schema#>",
                "SELECT *",
                "{ GRAPH ?g { ?s a <http://example.org/Entity> . ?s temporal:query 'foo' } }"
                ) ;
            Query q = QueryFactory.create(qs) ;
            QueryExecution qexec = QueryExecutionFactory.create(q, ds) ;
            ResultSet rs = qexec.execSelect() ;
    ...
    @Test public void textTxn_bnode_subject() {
        ...
        Txn.executeRead(ds,  ()->{
            String qs = StrUtils.strjoinNL(
                ...
                "PREFIX rdfs:   <http://www.w3.org/2000/01/rdf-schema#>",
                "SELECT *",
                "{ ?s temporal:query 'foo' . ?s a <http://example.org/Entity> }"
                ) ;
            Query q = QueryFactory.create(qs) ;
            QueryExecution qexec = QueryExecutionFactory.create(q, ds) ;
            ResultSet rs = qexec.execSelect() ;
    ...
    @Test public void textTxn_bnode_subject_bound_first() {
        ...
        Txn.executeRead(ds,  ()->{
            String qs = StrUtils.strjoinNL(
                ...
                "PREFIX rdfs:   <http://www.w3.org/2000/01/rdf-schema#>",
                "SELECT *",
                "{ ?s a <http://example.org/Entity> . ?s temporal:query 'foo' }"
                );
            Query q = QueryFactory.create(qs);
            QueryExecution qexec = QueryExecutionFactory.create(q, ds);
            ResultSet rs = qexec.execSelect();
