...
public class TestBuildTextDataset extends BaseTest
{
    ...
    public void buildText_01() {
        createAssembler("temporal-config.ttl") ;
    }

    @Test
    public void buildText_02() {
        Dataset ds = createAssembler("temporal-config-union.ttl") ;
    ...
    public void buildText_04() {
        Dataset ds = createAssembler("temporal-config.ttl") ;
        loadData(ds) ;
        queryData(ds) ;
    }
    ...
    public static void queryData(Dataset dataset) {
        String pre = StrUtils.strjoinNL("PREFIX : <http://example/>", "PREFIX temporal: <http://jena.apache.org/temporal#>",
                                        "PREFIX rdfs: <http://www.w3.org/2000/01/rdf-schema#>") ;

        String qs = StrUtils.strjoinNL("SELECT * ", " { ?s temporal:query (rdfs:label 'X1') ;", "      rdfs:label ?label",
                                       " }") ;
