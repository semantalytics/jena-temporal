 ...
 */
public abstract class AbstractTestDatasetWithGraphTemporalIndex extends AbstractTestDatasetWithTemporalIndex {
    ...
    public void testOneSimpleResultInGraph() {
        ...
        String queryString = StrUtils.strjoinNL(
                ...
                "SELECT ?s",
                "WHERE {",
                "  GRAPH ?g { ?s temporal:query ( rdfs:label 'testOneSimpleResult' 10 ) . }",
                "}"
                );
    ...
    public void testOneResultTwoGraphs() {
        ...
        String queryString = StrUtils.strjoinNL(
                ...
                "SELECT ?s",
                "WHERE {",
                "  GRAPH <http://example.org/modelA> { ?s temporal:query ( rdfs:label 'testOneResult' 10 ) . }",
                "}"
                );
        Set<String> expectedURIs = new HashSet<>() ;
    ...
    public void testORFromGraphs() {
        ...
        String queryString = StrUtils.strjoinNL(
                ...
                "SELECT ?s",
                "WHERE {",
                "  GRAPH <http://example.org/modelA> { ?s temporal:query ( rdfs:label 'testResultOne OR testResultTwo' 10 ) . }",
                "}"
                );
        Set<String> expectedURIs = new HashSet<>() ;
    ...
    public void testQueryFromDefaultGraph() {
        ...
        String queryString = StrUtils.strjoinNL(
                ...
                "SELECT ?s",
                "WHERE {",
                "    ?s temporal:query ( rdfs:label 'testOneResult' 10 ) .",
                "}"
                );
        Set<String> expectedURIs = new HashSet<>() ;
        expectedURIs.addAll( Arrays.asList(RESOURCE_BASE + "testResultInModelA")) ;
    ...
    public void testBnodeIdentifiedGraph() {
        ...
        String queryString = StrUtils.strjoinNL(
                ...
                "SELECT ?s",
                "WHERE {",
                "  GRAPH ?g { ?s temporal:query ( rdfs:label 'testResult' 10 ) . }",
                "}"
                );
        Set<String> expectedURIs = new HashSet<>() ;
